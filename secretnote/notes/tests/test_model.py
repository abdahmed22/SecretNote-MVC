...
class TestModel(TestCase):
    ...
    def test_note_is_assigned_created_date(self):
        ...
        self.assertEquals(self.note1.created.time().second, timezone.now().time().second)
    ...
    def test_is_deleted(self):
        self.assertEquals(True, self.note1.is_deleted) # since timezone.now() inside test file always behind timezone.now() inside model.
